...
describe('Calculation panel indicator updates after answers', () => {
	...
	it('Marks first question as correct when the right answer is submitted', async () => {
		...
		const input: HTMLElement = await screen.findByRole('textbox', { name: /answer-input/i });
		await user.type(input, '3');

		const submitButton: HTMLElement = await screen.findByRole('button', { name: /submit/i });
		await user.click(submitButton);

		const indicator1: HTMLElement = await screen.findByTestId('question-indicator-0');
		expect(within(indicator1).getByRole('img', { name: /Correct Question/i })).toBeInTheDocument();

		const indicator2: HTMLElement = await screen.findByTestId('question-indicator-1');
		expect(indicator2).toHaveClass(styles.in_progress);
		expect(within(indicator2).getByText('2')).toBeInTheDocument();

		const indicator3: HTMLElement = await screen.findByTestId('question-indicator-2');
